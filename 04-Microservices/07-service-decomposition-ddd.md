...
**DDD = Domain-Driven Design** — a way of designing software around the business domain. In microservices it guides how you decompose a system into well-bounded services rather than one large monolith.

## DDD in an E-commerce Site
...
```text
Catalog Context --Publishes--> Ordering Context
Ordering --OrderPlaced Event--> Payments Context
Ordering --OrderPlaced Event--> Inventory/Warehouse
Payments --PaymentSucceeded Event--> Shipping Context
Inventory --StockReserved Event--> Shipping
Customer Context --> Ordering
```

## DDD in an IoT Data Platform
...
```text
Device Management --DeviceRegistered--> Data Ingestion
Data Ingestion --TelemetryReceived--> Data Processing/Analytics
Data Processing --AnomalyDetected--> Alerts & Notifications
Alerts --AlertRaised--> Integration/APIs
User & Access Management --> Integration/APIs
```
...
## Understanding Contexts, Aggregates, and Entities
...
Examples:
- **E-commerce:** *Order Aggregate* → Root: `Order`, children: `OrderLineItem`, `Address`, `PaymentInfo`. Rule: an order cannot be placed without at least one line item.
- **IoT:** *Device Aggregate* → Root: `Device`, children: `FirmwareVersion`, `Settings`, `TelemetryConfig`. Rule: a device must have a valid ID before it can send telemetry.
