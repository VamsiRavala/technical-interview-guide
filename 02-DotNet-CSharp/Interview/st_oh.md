# ASP.NET & EF Core – Interview Notes

A compact handbook covering the topics you listed: architecture patterns, Entity Framework (EF/Core) fundamentals, SQL, ASP.NET MVC/Web API/Core, RESTful CRUD, middleware, and global exception handling—with code you can adapt quickly.
...
## 2) Purpose of **tracking** in EF(Core)
...
## 3) How to **manage data loss** in EF/Core
...
## 4) ASP.NET MVC **request life cycle** (high‑level)
...
## 5) Web API vs Core MVC vs “Core issues”
...
- Useful for readability, recursion (hierarchies), breaking complex queries.
...
## 7) WCF vs Web API / SQL Server quick notes
- **WCF**: SOAP/WS-* protocols, multiple bindings (net.tcp, msmq), contracts, service behaviors. Best for legacy enterprise or where SOAP/interoperability features required.
...
## 8) Create **RESTful APIs for CRUD** using EF(Core)
...
**Notes**
- Returns proper **HTTP status codes** and location header on create.
- Uses **AsNoTracking** for list endpoint.
...
### Option A: Built‑in handler + RFC7807 Problem Details
```csharp
// Program.cs
...
## 10) “Explain in detail about your code” – how to present
...
## 11) Implement **Middleware in ASP.NET Core**
...
## 12) Extras: testing, validation, and performance
...
**Feel free to copy this into a repo as `README.md`. Ask if you want a runnable sample with projects wired up.**
